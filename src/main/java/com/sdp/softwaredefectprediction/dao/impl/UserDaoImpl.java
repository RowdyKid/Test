...
public class UserDaoImpl extends BaseDao implements UserDao {
    ...
    public User queryUserByUsername(String username) {
        String sql = "select `id`,`username`,`password`,`email`,`del` from system_user where username = ?";
    ...
    public User queryUserByUsernameAndPassword(String username, String password) {
        String sql = "select `id`,`username`,`password`,`email`,`del`,`rid` from system_user where username = ? and password = ?";
        return queryForOne(User.class, sql, username,password);
    }

    @Override
    public int saveUser(User user) {
        String sql = "insert into system_user(`username`,`password`,`email`,`del`) values(?,?,?,?)";
        return update(sql, user.getUsername(),user.getPassword(),user.getEmail(),user.getDel());
    }
    ...
    public void updateUserEmailInfo(User user) {
        String sql="update system_user set email = ?  where id = ?";
        update(sql,user.getEmail(),user.getId());
    }
