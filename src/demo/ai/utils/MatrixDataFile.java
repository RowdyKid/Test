...
public class MatrixDataFile {
	...
	public MatrixDataFile(String sourceFile,int flag) throws Exception {
		setSourceFile(sourceFile);
		loadDatap();
		initDatap();
	}
	...
	public void loadDatap(){
		data = Common.readFileToArrayListp(sourceFile);
	}
	
	
	public void initDatap() throws Exception{
...
//			y[row][0] = Float.parseFloat(features[features.length-1]) ;
	...
	//特征缩放
	public int featureScaling() throws Exception{
		// 开始缩放
		float min,max,avg;
		int rows = Matrix.rows(x);//x的行数
		int cols = Matrix.cols(x);//x的列数
		int col;
		...
		for(col=0;col<cols;col++) {
			...
			for(int row=0;row<rows;row++) {
				// 求标准差
				sum2 += Math.sqrt((x[row][col] - avg) * (x[row][col] - avg));

			}
			float standardDivition = sum2 / (rows - 1);
	        // 标准化
	        for (int row = 0; row < rows; row++) {
	        	x[row][col] = (x[row][col] - avg) / standardDivition;
	        }
		}
		return col;
	}
